...
Welche Datenstruktur wir verwenden, ist wichtig, weil sie bestimmt, wie wir auf Daten **zugreifen**, sie **verändern** und **analysieren** können.
...
Datenstrukturen in R bauen in gewisser Weise aufeinander auf. Am Anfang steht der **Vektor**:  
...
- `numbers` stellen zufällige Zahlen dar, die ihr euch ausdenkt
- `chars` sollen einfach Namen sein, die euch einfallen
...
- und ob es offensichtliche Auffälligkeiten gibt.
...
Wir sehen zunächst, dass **RStudio** direkt zwischen <span style="color: darkred; font-weight: bold;">Values (einzelne Werte)</span> und <span style="color: green; font-weight: bold;">Data (Datensätzen, Tabellen und anderen Objekten)</span> unterscheden kann.

___

![](assets/images/datastr_environ2.png){width=100% style="border: 2px solid darkblue;"}

Wir schauen uns jetzt mal unseren Datensatzobjekt genauer an. <span style="color: green; font-weight: bold;">Klicke</span> einfach auf den Datensatz `df_csv` und neben deinem Script sollte sich nun der Datensatz öffnen.
...
Zurück zum Environment! Mit einem <span style="color: green; font-weight: bold;">Klick</span> auf den blauen Pfeil neben dem Objektnamen, öffnest du ein Dropdown mit mehr Informationen zu diesem Objekt **→** Struktur und Inhalte des Objekts!

___

![](assets/images/datastr_environ5.png){width=100% style="border: 2px solid darkblue;"}

Was wir nun erkennen ist ein logischer Aufbau des Objekts. Wir sehen <span style="color: green; font-weight: bold;">den Objektnamen als übergeordnete Struktur</span> und die Inhalte des Objekts, in diesem Fall, die Variablen als untergeordnete Struktur. Getrennt werden diese zwei **"Ebenen"** durch das **`$`**.
...
```
In eurem Output, sollten nun alle Altersangaben der Teilnehmden Personen dieser Studie als Vektor angezeigt werden.
...
Bevor wir mehrere Schritte kombinieren,  
...
1. Nenne das Objekt (Datensatz)
2. Reihe die Variablen aneinander, die ausgewählt werden sollen

Die Datenaufbereitung besteht meistens aus **mehreren Schritten:**

- Variablen auswählen
- Fälle filtern
- neue Variablen erstellen (Summenwerte)
...
Die Logik des Befehls **"Alternative 2"** ist wie folgt aufgebaut:

- `filter()` den neuen Datensatz, welcher durch `select()` erstellt wurde

**oder**

- Wähle erst die Variablen aus und filter anschließend (**Alternative 1**)
...
2. Erstelle einen neuen Datensatz `df_analyse` mit folgenden Variablen `Alter, Geschlecht, Bildung, Berufstätigkeit und Sozialeungleichheit`
