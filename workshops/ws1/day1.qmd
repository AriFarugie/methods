...
Speichere **alle Ergebnisse als eigene Objekte**  
(z. B. `se_y`, `skew_y`, `kurt_y`).
...
\[
\text{Schiefe} = \frac{1}{n} \sum \left( \frac{x - \bar{x}}{sd(x)} \right)^3
\]
...
\[
\text{Kurtosis} = \frac{1}{n} \sum \left( \frac{x - \bar{x}}{sd(x)} \right)^4
\]

Hinweis:  
Alle Bestandteile der Formeln lassen sich mit **Base-R-Funktionen** umsetzen.
:::
